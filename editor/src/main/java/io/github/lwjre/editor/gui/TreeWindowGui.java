...
public abstract class TreeWindowGui<N> extends WindowGui {
	 ...
	 * This function calls {@link TreeWindowGui#drawChildren(Object)} to draw the node's children.
	 *
	 * @param node The node to draw
	 */
	protected void drawNode(N node) {
		if(node.equals(Clipboard.getContent())) {
			ImGui.pushStyleColor(ImGuiCol.Text, 0.7f, 0.7f, 0.7f, 1.0f);
		}
		String label = this.getLabel(node);
		if(ImGui.treeNodeEx(label, this.getFlags(node))) {
			if(node.equals(Clipboard.getContent())) {
				ImGui.popStyleColor();
			}
	 ...
	 * This function should call {@link TreeWindowGui#drawNode(Object)} for every child of the given node.
	 *
