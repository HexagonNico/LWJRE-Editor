...
public class VectorFieldGui implements IFieldGui {
	...
	public boolean drawGui(Component component, Field field) throws IllegalAccessException {
		...
		if(range != null) {
			return this.dragGui(component, field, range.step(), range.min(), range.max());
		} else {
			return this.inputGui(component, field);
		}
	}
	 ...
	 */
	private boolean dragGui(Component component, Field field, float step, float min, float max) throws IllegalAccessException {
	 ...
	 * @throws IllegalAccessException If the field could not be accessed
	 */
	private boolean inputGui(Component component, Field field) throws IllegalAccessException {
		Object obj = field.get(component);
		if(obj instanceof Vec2f vector) {
			float[] ptr = {vector.x(), vector.y()};
